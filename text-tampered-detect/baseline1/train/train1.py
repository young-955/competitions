...
total_optim = torch.optim.SGD(filter(lambda p: p.requires_grad, total_model.parameters()), lr=2e-5, momentum=0.9, weight_decay=1e-5)
...
for epoch in range(epoch_num):
    for step, item in enumerate(total_dataloader):
        ...
        if gstep % eval_step == 0:
            recall = eval_pseudo(total_model, eval_trans_size, hard=True, pseudo_path='home/wuziyang/Projects/data/text_manipulation_detection/resnet_pseudo_label')
            ...
            if recall > grecall:
                # min_loss = gloss
                grecall = recall
                torch.save(total_model, f'model_loss_{gloss}_recall_{recall}.pth')
            gloss = 0
